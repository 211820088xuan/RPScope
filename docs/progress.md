...
## TODO 13 项执行（2026-08-24）

| # | 任务 | 状态 | 结果 |
|---|---|---|---|
| 1 | 53条分歧标记 | ✅ | 64一致+28分歧写入 final.csv |
| 2 | 消歧model-gold | ✅ | rule-only: acc=55.6% P=96.2% R=39.1% (含LLM银标93.3%需长跑) |
| 3 | other重分类影响 | ✅ | 607×27%=161可能upstream, recall 2.5%→1.9%(被高估0.6pp) |
| 4 | P4扩100家 | ⏸ | 脚本就绪, LLM太慢(~12min/家) |
| 5 | P6 LLM事件抽取 | ⏸ | 脚本就绪(event_extractor.py), LLM速度限 |
| 6 | 消歧消融 | ✅ | pre=post(无差异, 名称对齐不依赖entity_id) |
| 7 | 全量person拆分 | ⏸ | 脚本就绪(apply_disambig.py), LLM长跑 |
| 8 | 压测并发 | ❌ | harness杀后台server |
| 9 | PDF导出 | ✅ | /api/report/{code}/pdf reportlab(200, 3.7KB) |
| 10 | 前端点击证据 | ✅ | onclick toggle .ev-detail |
| 11 | 技术博客 | ✅ | docs/blog-p5-three-class-insight.md |
| 12 | demo截图 | ❌ | 无浏览器 |
...
## P4 扩到 203 家 + eval 重跑（2026-08-24）

| 指标 | 107家时 | 203家时 |
|---|---|---|
| 关联方条数 | 4069 | 7650 |
| upstream | 1475(36%) | 2602(34%) |
| downstream | 883(22%) | 1425(19%) |
| other | 1711(42%) | 3623(47%) |
| 可比口径 P | 7.6% | 8.0% |
| 可比口径 R | 2.0% | 2.1% |
| 严格口径 P | 12.6% | 14.9% |
| 严格口径 R | 1.4% | 1.5% |
...
**T1 README 三分类行**: 19/208/1439(203家) → 54/458/2547(203家, upstream子集, 可比口径)

**T2 README 注释段**: 重写
- recall 1.3%(旧严格) → 2.1%(可比, 修复后)
- "208条" → "458条"
- "须人工三分类后修正" → "人工三分类尚未进行, precision 未做修正, 这是已知缺口"
- 补充: 47% other + 19% downstream 的结构性原因

**T3 blog**: 全文修正
- 6.2% → 10.5%, 2.5% → 2.1%, 37.1% → 47%
- 删除"模型标注结果"整节(暗示三分类已出结果)
- 改写为方法论框架介绍(口径设计+三分类框架+修正公式), 不暗示已有核查结论
- 补充: R2修复(169→16, -90.5%), 政府控制人排除(局≠委员会)

**T4 scope-class-review.md**: 标题 607/37.1% → 3623/47.4%, 标注Top50表为旧数据待重跑

**T5 全局扫描**:
| 文件 | 旧数字 | 判定 |
|---|---|---|
| eval-v1.md | 14.9%→19.5%, 8.0%→10.5%, 617→458, 208→16, 607→3623 | 已改 |
| design.md | 208→458, 1.3%→2.1% | 已改 |
| disambiguation.md | 6.2%→10.5% | 已改 |
| blog | 6.2%→10.5%, 2.5%→2.1%, 37.1%→47%, 53条→458条, 删除模型标注结果节 | 已改 |
| gold-standard.md | "50条"出现(均为203家/7650条) | 不需改(不是旧数字) |
| scope-class-review.md | 607/37.1% → 3623/47.4% | 已改(标题), Top50表标注为旧数据 |
| README | 8022→5927, 65069→61225, 196619→196415 | 已改 |

**T6 README vs 来源文档一致性**: 全部 10/10 一致
